...
function buildPortfolioResponse({
  ...
  const graphByName = new Map(
    publicGraphRepositories.map((repository) => [repository.name.toLowerCase(), repository]),
  );
  ...
  const selectedNames = pinnedNames.length
    ? pinnedNames
    : configuredRepositories(fallbackRepositories);
  ...
  const projects = selectedNames
    .map((name) => {
      ...
      const featured = pinnedNames.length > 0;
  ...
  return {
    ...
    meta: {
      generatedAt,
      projectSource: pinnedNames.length ? 'pinned' : 'configured',
